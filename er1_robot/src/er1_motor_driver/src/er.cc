...
ER::ER()
{
  ...
  _axle_length = ER_DEFAULT_AXLE_LENGTH;
...
//  ROS_INFO("read:\n");
//  for(size_t i=0;i<numread;i++) ROS_INFO("0x%02x\n", s[i]);
...
ER::send_command( unsigned char address, unsigned char c, int ret_num, unsigned char * ret )
{
  ...
  if( result < 0 )
  {
    ROS_INFO( "failed to send command\n" );
  }
  ...
  if( ret > 0 )
  {
    ...
    {
      ROS_INFO( "failed to read response\n" );
    }
  }
...
ER::InitRobot()
{
  ...
  {
    ROS_INFO("failed to initialize robot\n");
    return -1;
  }
...
ER::Setup()
{
  ...
  serial_info.custom_divisor = (int)(24000000.0/250000.0 + 0.5);
  
  if( ioctl( this->_fd, TIOCSSERIAL, &serial_info ) < 0)
  {
...
ER::Shutdown()
{
  ...
  if(SetVelocity(0,0) < 0)
    ROS_INFO("failed to stop robot while shutting down\n");
  

  usleep(ER_DELAY_US);

  if(close(this->_fd))
    ROS_INFO("close() failed:%s\n",strerror(errno));
  this->_fd = -1;
  if( _debug )
    ROS_INFO("ER has been shutdown\n\n");
...
//     ROS_INFO("got motor power req: %d\n", powercfg->state);
...
// //	ROS_INFO( "position_cmd.xspeed: %d position_cmd.yawspeed: %d\n", position_cmd.xspeed, position_cmd.yawspeed );
...
//   if( _debug )
//     ROS_INFO( "final_lvel: %f, final_rvel: %f\n", final_lvel, final_rvel );
...
//	ROS_INFO( "speed: %f checksum: 0x%02x value: 0x%08x\n", speed, cmd[1], whole );
...
*/
  ...
  timediff = (currtime.tv_sec + currtime.tv_usec/1.0e6)-
